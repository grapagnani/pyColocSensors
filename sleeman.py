...
def sleeman(stream):
    ...
    (P01, f) = mlab.csd(m[0].data, m[1].data, Fs=fs, NFFT=n_fft,
                        noverlap=n_overlap, detrend=detrend_func,
                        window=cosine_taper(n_fft, p=0.2),
                        scale_by_freq=True)

    (P02, f) = mlab.csd(m[0].data, m[2].data, Fs=fs, NFFT=n_fft,
                        noverlap=n_overlap, detrend=detrend_func,
                        window=cosine_taper(n_fft, p=0.2),
                        scale_by_freq=True)
    ...
    (P12, f) = mlab.csd(m[1].data, m[2].data, Fs=fs, NFFT=n_fft,
                        noverlap=n_overlap, detrend=detrend_func,
